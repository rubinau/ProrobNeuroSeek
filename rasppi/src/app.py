...
# Import both send and receive functions from robot_control
from robot_control import send_command_to_mcu, get_latest_mcu_message 
...
app_state = { 
    ...
    "best_nlp_score": 0.0, 
    "ignored_items": set(), 
    "mcu_message": "N/A", # <-- ADDED: To store the latest message from the MCU
    "lock": threading.Lock() 
} 
...
def background_processing_loop(): 
    ...
    while True: 
        # --- Check for incoming MCU data ---
        mcu_msg = get_latest_mcu_message()
        if mcu_msg:
            with app_state['lock']:
                app_state['mcu_message'] = mcu_msg
        ...
        with app_state['lock']: 
            ...
            status_data.pop('lock') 
            status_data['ignored_items'] = list(status_data['ignored_items']) 
...
def print_console_status(state): 
    ...
    print(f"  - Last MCU Message : {state['mcu_message']}") # <-- ADDED
    print("="*60 + "\n") 
...
def handle_set_target(json_data): 
    ...
    with app_state['lock']: 
        ...
        app_state['ignored_items'].clear() 
    send_command_to_mcu('i')
